MODULE integration
   ...
   CONTAINS
      ...
      SUBROUTINE vegas(region,func,init,ncall,itmx,nprn,tgral,sd,chi2a)
         REAL(KIND=dp), DIMENSION(:), INTENT(IN) :: region
         INTEGER(KIND=4), INTENT(IN) :: init,ncall,itmx,nprn
         REAL(KIND=dp), INTENT(OUT) :: tgral,sd,chi2a
         INTERFACE
            REAL(KIND=dp) FUNCTION func(pt,wgt)
               USE kinds
               IMPLICIT NONE
            ...
            END FUNCTION func
         END INTERFACE
         REAL(KIND=dp), PARAMETER :: ALPH=1.5_dp,TINY=1E-30_dp
         INTEGER(KIND=4), PARAMETER :: MXDIM=10,NDMX=50
         INTEGER(KIND=4), SAVE :: i,it,j,k,mds,nd,ndim,ndo,ng,npg
         INTEGER(KIND=4), DIMENSION(MXDIM), SAVE :: ia,kg
         REAL(KIND=dp), SAVE :: calls,dv2g,dxg,f,f2,f2b,fb,rc,ti,tsi,wgt,xjac,xn,xnd,xo,harvest
         REAL(KIND=dp), DIMENSION(NDMX,MXDIM), SAVE :: d,di,xi
         REAL(KIND=dp), DIMENSION(MXDIM), SAVE :: dt,dx,x
         REAL(KIND=dp), DIMENSION(NDMX), SAVE :: r,xin
         REAL(KIND=dp), SAVE :: schi,si,swgt
         ndim=size(region)/2
         if (init <= 0) then
            mds=1
            ndo=1
      ...
      !BL
            ...
            REAL(KIND=dp), DIMENSION(:), INTENT(OUT) :: xin
            REAL(KIND=dp), DIMENSION(:), INTENT(INOUT) :: xi
            INTEGER(KIND=4) :: i,k
            REAL(KIND=dp) :: dr,xn,xo
            k=0
            xo=0.0
            dr=0.0
            do i=1,nd-1
