...
neutrino_pdfs_mu_dpm_El = np.loadtxt("fit_dpmjet/El_fit/mu_pdf.txt", delimiter=",")
neutrino_pdfs_mub_dpm_El = np.loadtxt("fit_dpmjet/El_fit/mub_pdf.txt", delimiter=",")

neutrino_pdfs_mu_qgs_El = np.loadtxt("fit_qgsjet/El_fit/mu_pdf.txt", delimiter=",")
neutrino_pdfs_mub_qgs_El = np.loadtxt("fit_qgsjet/El_fit/mub_pdf.txt", delimiter=",")

neutrino_pdfs_mu_epos_El = np.loadtxt("fit_epos/El_fit/mu_pdf.txt", delimiter=",")
neutrino_pdfs_mub_epos_El = np.loadtxt("fit_epos/El_fit/mub_pdf.txt", delimiter=",")

neutrino_pdfs_mu_sib_El = np.loadtxt("fit_sibyll/El_fit/mu_pdf.txt", delimiter=",")
neutrino_pdfs_mub_sib_El = np.loadtxt("fit_sibyll/El_fit/mub_pdf.txt", delimiter=",")
...
def plot(
    ...
    mean_mu_qgs_El = np.mean(neutrino_pdfs_mu_qgs_El, axis=0) * x_vals
    error_mu_qgs_El = np.std(neutrino_pdfs_mu_qgs_El, axis=0) * x_vals
    mean_mub_qgs_El = np.mean(neutrino_pdfs_mub_qgs_El, axis=0) * x_vals
    error_mub_qgs_El = np.std(neutrino_pdfs_mub_qgs_El, axis=0) * x_vals

    mean_mu_epos_El = np.mean(neutrino_pdfs_mu_epos_El, axis=0) * x_vals
    error_mu_epos_El = np.std(neutrino_pdfs_mu_epos_El, axis=0) * x_vals
    mean_mub_epos_El = np.mean(neutrino_pdfs_mub_epos_El, axis=0) * x_vals
    error_mub_epos_El = np.std(neutrino_pdfs_mub_epos_El, axis=0) * x_vals

    mean_mu_sib_El = np.mean(neutrino_pdfs_mu_sib_El, axis=0) * x_vals
    error_mu_sib_El = np.std(neutrino_pdfs_mu_sib_El, axis=0) * x_vals
    mean_mub_sib_El = np.mean(neutrino_pdfs_mub_sib_El, axis=0) * x_vals
    error_mub_sib_El = np.std(neutrino_pdfs_mub_sib_El, axis=0) * x_vals
    ...
    axLqgs_El_err = axL.fill_between(
        x_vals,
        mean_mu_qgs_El + error_mu_qgs_El,
        mean_mu_qgs_El - error_mu_qgs_El,
        color="black",
        alpha=0.2,
        label=r"$\pm 1\sigma$",
    ...
    axLepos_El_err = axL.fill_between(
        x_vals,
        mean_mu_epos_El + error_mu_epos_El,
        mean_mu_epos_El - error_mu_epos_El,
        color="yellow",
        alpha=0.2,
        label=r"$\pm 1\sigma$",
    ...
    axLsib_El_err = axL.fill_between(
        x_vals,
        mean_mu_sib_El + error_mu_sib_El,
        mean_mu_sib_El - error_mu_sib_El,
        color="blue",
        alpha=0.2,
        label=r"$\pm 1\sigma$",
    ...
    axL.legend(
        ...
        [
            ...
            (axLqgs_El_err, axLqgs_El),
            (axLepos_El_err, axLepos_El),
            (axLsib_El_err, axLsib_El),
        ],
        [
            # r"$f_{\nu_\mu, \mathrm{ref \ (EPOS+PWG)}}$",
            # r"$f_{\nu_\mu, \mathrm{EPOS+POWHEG} \ E_\nu}$",
            r"$f_{\nu_\mu, \mathrm{DPM}\ E_l}$",
            r"$f_{\nu_\mu, \mathrm{QGS}\ E_l}$",
            r"$f_{\nu_\mu, \mathrm{EPOS}\ E_l}$",
            r"$f_{\nu_\mu, \mathrm{SIBYLL}\ E_l}$",
        ],
    ...
    axL.set_xlim(5e-4, 1)
    axL.set_ylim(1e-1, 1e8)
    axL.set_yscale("log")
    axL.set_xscale("log")

    # if preproc == 1:
    title_str = r"$ El \ \mathrm{EPOS} \ + \ \mathrm{QGSJET} + \mathrm{DPMJET}$"
    # if preproc == 2:
    ...
    axMqgs_El_err = axM.fill_between(
        x_vals,
        (mean_mub_qgs_El + error_mub_qgs_El),
        (mean_mub_qgs_El - error_mub_qgs_El),
        color="black",
        alpha=0.2,
        label=r"$\pm 1\sigma$",
    ...
    axMepos_El_err = axM.fill_between(
        x_vals,
        (mean_mub_epos_El + error_mub_epos_El),
        (mean_mub_epos_El - error_mub_epos_El),
        color="yellow",
        alpha=0.2,
        label=r"$\pm 1\sigma$",
    ...
    axMsib_El_err = axM.fill_between(
        x_vals,
        (mean_mub_sib_El + error_mub_sib_El),
        (mean_mub_sib_El - error_mub_sib_El),
        color="blue",
        alpha=0.2,
        label=r"$\pm 1\sigma$",
    ...
    axM.legend(
        ...
        [
            ...
            (axMqgs_El_err, axMqgs_El),
            (axMepos_El_err, axMepos_El),
            (axMsib_El_err, axMsib_El),
        ],
        [
            # r"$f_{\bar{\nu}_\mu, \mathrm{ref} \ (EPOS+PWG)}$",
            # r"$f_{\bar{\nu}_\mu, \mathrm{EPOS+POWHEG} \ E_\nu}$",
            r"$f_{\bar{\nu}_\mu, \mathrm{DPM}\ E_l}$",
            r"$f_{\bar{\nu}_\mu, \mathrm{QGS}\ E_l}$",
            r"$f_{\bar{\nu}_\mu, \mathrm{EPOS}\ E_l}$",
            r"$f_{\bar{\nu}_\mu, \mathrm{SIBYLL}\ E_l}$",
        ],
        handler_map={tuple: HandlerTuple(ndivide=1)},
        loc="lower left",
    ).set_alpha(0.8)

    title_str = r"$\mathcal{L}_{\mathrm{pp}} = 3 \mathrm{ab}^{-1}, \ \mathrm{FASERv2} $"
    # if preproc == 2:
    # title_str = rf"$f_{{\mathrm{{NN}}}}(x) = \mathrm{{NN}}_{{{layers}}}(x) -  \mathrm{{NN}}_{{{layers}}}(1)$"
    ...
    ratio_center = mean_mu_qgs_El / faser_pdf_mu
    ratio_lower = (mean_mu_qgs_El - error_mu_qgs_El) / faser_pdf_mu
    ratio_upper = (mean_mu_qgs_El + error_mu_qgs_El) / faser_pdf_mu
    ...
    ratio_center = mean_mu_epos_El / faser_pdf_mu
    ratio_lower = (mean_mu_epos_El - error_mu_epos_El) / faser_pdf_mu
    ratio_upper = (mean_mu_epos_El + error_mu_epos_El) / faser_pdf_mu
    ...
    ratio_center = mean_mu_sib_El / faser_pdf_mu
    ratio_lower = (mean_mu_sib_El - error_mu_sib_El) / faser_pdf_mu
    ratio_upper = (mean_mu_sib_El + error_mu_sib_El) / faser_pdf_mu

    axrL.plot(x_vals, ratio_center, linestyle="-", color="blue")
    axrL.fill_between(x_vals, ratio_upper, ratio_lower, color="blue", alpha=0.2)
    ...
    axrM.plot(x_vals, mean_mub_dpm_El / faser_pdf_mub, linestyle="-", color="green")
    axrM.fill_between(
        x_vals,
    ...
    axrM.plot(x_vals, mean_mub_qgs_El / faser_pdf_mub, linestyle="-", color="black")
    axrM.fill_between(
        x_vals,
        (mean_mub_qgs_El + error_mub_qgs_El) / faser_pdf_mub,
        (mean_mub_qgs_El - error_mub_qgs_El) / faser_pdf_mub,
        color="black",
        alpha=0.2,
    )

    axrM.plot(x_vals, mean_mub_epos_El / faser_pdf_mub, linestyle="-", color="yellow")
    axrM.fill_between(
        x_vals,
        (mean_mub_epos_El + error_mub_epos_El) / faser_pdf_mub,
        (mean_mub_epos_El - error_mub_epos_El) / faser_pdf_mub,
        color="yellow",
        alpha=0.2,
    )

    axrM.plot(x_vals, mean_mub_sib_El / faser_pdf_mub, linestyle="-", color="blue")
    axrM.fill_between(
        x_vals,
        (mean_mub_sib_El + error_mub_sib_El) / faser_pdf_mub,
        (mean_mub_sib_El - error_mub_sib_El) / faser_pdf_mub,
        color="blue",
        alpha=0.2,
    )
