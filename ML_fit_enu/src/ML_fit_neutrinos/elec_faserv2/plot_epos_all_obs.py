...
neutrino_pdfs_mu_dpm_Eh = np.loadtxt(
    f"fit_{generator}/Eh_fit/mu_pdf.txt", delimiter=","
)
neutrino_pdfs_mub_dpm_Eh = np.loadtxt(
    f"fit_{generator}/Eh_fit/mub_pdf.txt", delimiter=","
)

neutrino_pdfs_mu_dpm_El = np.loadtxt(
    f"fit_{generator}/El_fit/mu_pdf.txt", delimiter=","
)
neutrino_pdfs_mub_dpm_El = np.loadtxt(
    f"fit_{generator}/El_fit/mub_pdf.txt", delimiter=","
)

neutrino_pdfs_mu_dpm_Enu = np.loadtxt(
    f"fit_{generator}/Enu_fit/mu_pdf.txt", delimiter=","
)
neutrino_pdfs_mub_dpm_Enu = np.loadtxt(
    f"fit_{generator}/Enu_fit/mub_pdf.txt", delimiter=","
)

neutrino_pdfs_mu_dpm_theta = np.loadtxt(
    f"fit_{generator}/theta_fit/mu_pdf.txt", delimiter=","
)
neutrino_pdfs_mub_dpm_theta = np.loadtxt(
    f"fit_{generator}/theta_fit/mub_pdf.txt", delimiter=","
)
...
def plot(
    ...
    axL.legend(
        ...
        [
            ...
            r"$f_{\nu_\mu, \mathrm{EPOS+PWG}\ E_h}$",
            r"$f_{\nu_\mu, \mathrm{EPOS+PWG}\ E_l}$",
            r"$f_{\nu_\mu, \mathrm{EPOS+PWG}\ E_\nu}$",
            r"$f_{\nu_\mu, \mathrm{EPOS+PWG}\ \theta}$",
        ],
    ...
    # if preproc == 1:
    title_str = r"$\mathrm{EPOS} \ + \ \mathrm{PWG} $"
    # if preproc == 2:
    ...
    axM.legend(
        ...
        [
            ...
            r"$f_{\bar{\nu}_\mu, \mathrm{EPOS+PWG}\ E_h}$",
            r"$f_{\bar{\nu}_\mu, \mathrm{EPOS+PWG} \ E_l}$",
            r"$f_{\bar{\nu}_\mu, \mathrm{EPOS+PWG} \ E_\nu}$",
            r"$f_{\bar{\nu}_\mu, \mathrm{EPOS+PWG} \ \theta}$",
        ],
        handler_map={tuple: HandlerTuple(ndivide=1)},
        loc="lower left",
    ).set_alpha(0.8)
    ...
    axrM.plot(x_vals, mean_mub_dpm_Enu / faser_pdf_mub, linestyle="-", color="blue")
    axrM.fill_between(
        x_vals,
    ...
    plt.savefig(f"{generator}_fits_obs.pdf")
    plt.show()
