...
neutrino_pdfs_mu_qgs_theta = np.loadtxt(
    "fit_qgsjet/theta_fit/mu_pdf.txt", delimiter=","
)
neutrino_pdfs_mub_qgs_theta = np.loadtxt(
    "fit_qgsjet/theta_fit/mub_pdf.txt", delimiter=","
)

neutrino_pdfs_mu_epos_theta = np.loadtxt("fit_epos/theta_fit/mu_pdf.txt", delimiter=",")
neutrino_pdfs_mub_epos_theta = np.loadtxt(
    "fit_epos/theta_fit/mub_pdf.txt", delimiter=","
)
...
def plot(
    ...
    mean_mu_qgs_theta = np.mean(neutrino_pdfs_mu_qgs_theta, axis=0) * x_vals
    error_mu_qgs_theta = np.std(neutrino_pdfs_mu_qgs_theta, axis=0) * x_vals
    mean_mub_qgs_theta = np.mean(neutrino_pdfs_mub_qgs_theta, axis=0) * x_vals
    error_mub_qgs_theta = np.std(neutrino_pdfs_mub_qgs_theta, axis=0) * x_vals

    mean_mu_epos_theta = np.mean(neutrino_pdfs_mu_epos_theta, axis=0) * x_vals
    error_mu_epos_theta = np.std(neutrino_pdfs_mu_epos_theta, axis=0) * x_vals
    mean_mub_epos_theta = np.mean(neutrino_pdfs_mub_epos_theta, axis=0) * x_vals
    error_mub_epos_theta = np.std(neutrino_pdfs_mub_epos_theta, axis=0) * x_vals
    ...
    axLqgs_theta_err = axL.fill_between(
        x_vals,
        mean_mu_qgs_theta + error_mu_qgs_theta,
        mean_mu_qgs_theta - error_mu_qgs_theta,
        color="black",
        alpha=0.2,
        label=r"$\pm 1\sigma$",
    ...
    axLepos_theta_err = axL.fill_between(
        x_vals,
        mean_mu_epos_theta + error_mu_epos_theta,
        mean_mu_epos_theta - error_mu_epos_theta,
        color="yellow",
        alpha=0.2,
        label=r"$\pm 1\sigma$",
    ...
    axL.legend(
        ...
        [
            ...
            (axLqgs_theta_err, axLqgs_theta),
            (axLepos_theta_err, axLepos_theta),
        ],
        [
            # r"$f_{\nu_\mu, \mathrm{ref \ (EPOS+PWG)}}$",
            # r"$f_{\nu_\mu, \mathrm{EPOS+POWHEG} \ E_\nu}$",
            r"$f_{\nu_\mu, \mathrm{DPM}\ \theta}$",
            r"$f_{\nu_\mu, \mathrm{QGS}\ \theta}$",
            r"$f_{\nu_\mu, \mathrm{EPOS}\ \theta}$",
        ],
    ...
    # if preproc == 1:
    title_str = r"$ \theta \ \mathrm{EPOS} \ + \ \mathrm{QGSJET} + \mathrm{DPMJET}$"
    # if preproc == 2:
    ...
    axMqgs_theta_err = axM.fill_between(
        x_vals,
        (mean_mub_qgs_theta + error_mub_qgs_theta),
        (mean_mub_qgs_theta - error_mub_qgs_theta),
        color="black",
        alpha=0.2,
        label=r"$\pm 1\sigma$",
    )
    (axMqgs_theta,) = axM.plot(
        x_vals,
        mean_mub_qgs_theta,
        linestyle="-",
        color="black",
    ...
    axMepos_theta_err = axM.fill_between(
        x_vals,
        (mean_mub_epos_theta + error_mub_epos_theta),
        (mean_mub_epos_theta - error_mub_epos_theta),
        color="yellow",
        alpha=0.2,
        label=r"$\pm 1\sigma$",
    )
    (axMepos_theta,) = axM.plot(
        x_vals,
        mean_mub_epos_theta,
        linestyle="-",
        color="yellow",
    ...
    axM.legend(
        ...
        [
            ...
            (axMqgs_theta_err, axMqgs_theta),
            (axMepos_theta_err, axMepos_theta),
        ],
        [
            # r"$f_{\bar{\nu}_\mu, \mathrm{ref} \ (EPOS+PWG)}$",
            # r"$f_{\bar{\nu}_\mu, \mathrm{EPOS+POWHEG} \ E_\nu}$",
            r"$f_{\bar{\nu}_\mu, \mathrm{DPM}\ \theta}$",
            r"$f_{\bar{\nu}_\mu, \mathrm{QGS}\ \theta}$",
            r"$f_{\bar{\nu}_\mu, \mathrm{EPOS}\ \theta}$",
        ],
        handler_map={tuple: HandlerTuple(ndivide=1)},
        loc="lower left",
    ).set_alpha(0.8)

    title_str = r"$\mathcal{L}_{\mathrm{pp}} = 150 \mathrm{fb}^{-1}, \ \mathrm{FASER} \ \mathrm{FASER} \ 2412$"
    # if preproc == 2:
    # title_str = rf"$f_{{\mathrm{{NN}}}}(x) = \mathrm{{NN}}_{{{layers}}}(x) -  \mathrm{{NN}}_{{{layers}}}(1)$"
    ...
    ratio_center = mean_mu_qgs_theta / faser_pdf_mu
    ratio_lower = (mean_mu_qgs_theta - error_mu_qgs_theta) / faser_pdf_mu
    ratio_upper = (mean_mu_qgs_theta + error_mu_qgs_theta) / faser_pdf_mu
    ...
    ratio_center = mean_mu_epos_theta / faser_pdf_mu
    ratio_lower = (mean_mu_epos_theta - error_mu_epos_theta) / faser_pdf_mu
    ratio_upper = (mean_mu_epos_theta + error_mu_epos_theta) / faser_pdf_mu
    ...
    axrM.plot(x_vals, mean_mub_dpm_theta / faser_pdf_mub, linestyle="-", color="green")
    axrM.fill_between(
        x_vals,
    ...
    axrM.plot(x_vals, mean_mub_qgs_theta / faser_pdf_mub, linestyle="-", color="black")
    axrM.fill_between(
        x_vals,
        (mean_mub_qgs_theta + error_mub_qgs_theta) / faser_pdf_mub,
        (mean_mub_qgs_theta - error_mub_qgs_theta) / faser_pdf_mub,
        color="black",
        alpha=0.2,
    )
    ...
    axrM.plot(
        x_vals, mean_mub_epos_theta / faser_pdf_mub, linestyle="-", color="yellow"
    )
    axrM.fill_between(
        x_vals,
        (mean_mub_epos_theta + error_mub_epos_theta) / faser_pdf_mub,
        (mean_mub_epos_theta - error_mub_epos_theta) / faser_pdf_mub,
        color="yellow",
        alpha=0.2,
    )
    ...
    plt.savefig("theta_comparison.pdf")
    plt.show()
