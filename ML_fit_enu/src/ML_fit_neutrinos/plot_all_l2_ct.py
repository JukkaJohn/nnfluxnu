...
import sys
import lhapdf
from data_for_plot import data_needed_for_fit
    ...
    for geometry in geometries:
        for generator in generators:
            for obs in observables:
                # if geometry == "run_3_gens" and obs == "theta" and generator == "epos":
                #     break
                # if geometry == "run_3_gens" and obs == "Enu" and generator == "qgjset":
                #     break
                print("now plotting")
                ...
                print(level1)
                level1 = level1[0].detach().numpy().flatten()
                print(data_mu)
                data_mub = level1[len(data_mu) :]
                print(f"data_mub = {data_mub}")
                data_mu = level1[: len(data_mu)]

                faser_pdf_mu, x_faser = read_pdf(pdf, x_vals, 14)
                faser_pdf_mub, x_faser = read_pdf(pdf, x_vals, -14)
                faser_pdf_mu = faser_pdf_mu * x_vals * factor * 1.16186e-09
                faser_pdf_mub = faser_pdf_mub * x_vals * factor * 1.16186e-09

                neutrino_pdfs_mu = np.loadtxt(
                    f"{geometry}/fit_{generator}/{obs}_fit/mu_pdf.txt", delimiter=","
                )
                neutrino_pdfs_mub = np.loadtxt(
                    f"{geometry}/fit_{generator}/{obs}_fit/mub_pdf.txt", delimiter=","
                )
                N_event_pred_mu = np.loadtxt(
                    f"{geometry}/fit_{generator}/{obs}_fit/events_mu.txt", delimiter=","
                )
                ...
                simulated_Enu = data_mu * factor

                errors_enu = np.sqrt(data_mu * factor)
                ...
                # title_str = r"$\mathrm{DPMJET} \ 2024 \ \mathrm{FASER} \ E_\nu \ L2 \ \mathrm{Closure} \ \mathrm{Test}$"
                ...
                plt.savefig(f"{geometry}_{generator}_{obs}_l2_plot.pdf")
                plt.show()
