...
from form_loss_fct import raw_loss_fct
...
from plot_dpmjet import plot

# Get number of reps from make runscripts
filename_postfit = "fit_report_dpm_Enu.txt"
(
    preproc,
    lr,
    fit_level,
...
filename_data_mu = (
    "../../../FKtables/data/data/data_Enu_FASERv_Run3_DPMJET+DPMJET_7TeV_numu_W.dat"
)
filename_data_mub = (
    "../../../FKtables/data/data/data_Enu_FASERv_Run3_DPMJET+DPMJET_7TeV_nubmu_W.dat"
)
filename_uncert_mu = "../../../FKtables/data/uncertainties/uncertainties_Enu_FASERv_Run3_DPMJET+DPMJET_7TeV_numu_W.dat"
filename_uncert_mub = "../../../FKtables/data/uncertainties/uncertainties_Enu_FASERv_Run3_DPMJET+DPMJET_7TeV_nubmu_W.dat"
filename_fk_mub_n = "../../../FKtables/data/fastkernel/FK_Enu_fine_nubmu_n.dat"
filename_fk_mub_p = "../../../FKtables/data/fastkernel/FK_Enu_fine_nubmu_p.dat"
filename_fk_mu_n = "../../../FKtables/data/fastkernel/FK_Enu_fine_numu_n.dat"
filename_fk_mu_p = "../../../FKtables/data/fastkernel/FK_Enu_fine_numu_p.dat"
filename_binsize = "../../../FKtables/data/binning/FK_Enu_fine_binsize.dat"
...
# # plt.plot(low_bin, data_mub, label="before")
x, faser = read_pdf(pdf, x_alphas, 14)

indices = np.abs(x_vals[:, None] - x_alphas.detach().numpy().flatten()).argmin(axis=0)
print(f"indices = {indices}")

neutrino_mu = np.mean(neutrino_pdfs_mu, axis=0)
print(neutrino_mu[indices].shape)
neutrino_mu = neutrino_mu[indices]
print(neutrino_mu.shape)
neutrino_mu = torch.tensor(neutrino_mu, dtype=torch.float32)
print(neutrino_mu.shape)
print(binwidths_mu.shape)
events_mu = torch.matmul(fk_tables_mu, neutrino_mu) * binwidths_mu.flatten()
print(events_mu.shape)

neutrino_mub = np.mean(neutrino_pdfs_mub, axis=0)
print(neutrino_mub[indices].shape)
neutrino_mub = neutrino_mub[indices]
print(neutrino_mub.shape)
neutrino_mub = torch.tensor(neutrino_mub, dtype=torch.float32)
print(neutrino_mub.shape)
print(binwidths_mub.shape)
events_mub = torch.matmul(fk_tables_mub, neutrino_mub) * binwidths_mub.flatten()
print(events_mub.shape)


events = torch.hstack((events_mu.flatten(), events_mub.flatten()))
data = torch.tensor(data, dtype=torch.float32)
diff = events - data
print(cov_matrix.shape)
print(diff.shape)
diffcov = torch.matmul(cov_matrix, diff)
loss = (1 / data.size(0)) * torch.dot(diff.view(-1), diffcov.view(-1))
print(f"loss = {loss}")
print(indices)
...
def compute_postfit_measures(cov_matrix, N_event_pred, data, level1, pred):
    ...
    if fit_level != 0:
        ...
        accuracy = compute_postfit.compute_accuracy(
            x_alphas.detach().numpy().flatten(), neutrino_pdfs_mu, pdf, 1.5
        )
