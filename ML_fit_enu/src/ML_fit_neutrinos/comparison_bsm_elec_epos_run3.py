...
def plot(
    ...
    for obs in observables:
        ...
        neutrino_pdfs_mu_bsm_eta = np.loadtxt(
            f"elec_bsm_eta/{obs}_fit/mu_pdf.txt", delimiter=","
        )
        ...
        neutrino_pdfs_mu_bsm_eta_prime = np.loadtxt(
            f"elec_bsm_eta_prime/{obs}_fit/mu_pdf.txt", delimiter=","
        )
        neutrino_pdfs_mub_bsm_eta_prime = np.loadtxt(
            f"elec_bsm_eta_prime/{obs}_fit/mub_pdf.txt", delimiter=","
        )

        neutrino_pdfs_mu_bsm_pi = np.loadtxt(
            f"elec_bsm_pi/{obs}_fit/mu_pdf.txt", delimiter=","
        )
        neutrino_pdfs_mub_bsm_pi = np.loadtxt(
            f"elec_bsm_pi/{obs}_fit/mub_pdf.txt", delimiter=","
        )
        ...
        gs.update(
            left=0.05, right=0.97, top=0.92, bottom=0.08, hspace=0.15, wspace=0.15
        )
        ...
        axL.legend(
            ...
            [
                r"$\mathrm{EPOS}$",
                r"$\mathrm{BSM}(\eta \rightarrow \mu)$",
                r"$\mathrm{BSM}(\eta^{'} \rightarrow \mu)$",
                r"$\mathrm{BSM}(\pi \rightarrow \mu)$",
            ],
        ...
        axLsig.plot(
            x_vals,
            np.abs(mean_mu_bsm_eta - faser_pdf_mu_epos) / error_mu_bsm_eta,
            linestyle="--",
            color="#fe6100",
        )
        # axLsig.fill_between(
        ...
        axLsig.plot(
            x_vals,
            np.abs(mean_mu_bsm_eta_prime - faser_pdf_mu_epos) / error_mu_bsm_eta_prime,
            linestyle="-",
            color="#dc267f",
        )
        # axLsig.fill_between(
        ...
        axLsig.plot(
            x_vals,
            np.abs(mean_mu_bsm_pi - faser_pdf_mu_epos) / error_mu_bsm_pi,
            linestyle=":",
            color="#000000",
        )
        # axLsig.fill_between(
        ...
        plt.savefig(f"{obs}_compare_bsm_elec_eta.pdf")
        # plt.show()
