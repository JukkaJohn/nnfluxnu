    ...
    for obs in observables:
        for geometry in geometries:
            ...
            neutrino_pdfs_mu_dpm = np.loadtxt(
                f"{geometry}/fit_dpmjet/{obs}_fit/pdf.txt", delimiter=","
            )
            ...
            neutrino_pdfs_mu_epos = np.loadtxt(
                f"{geometry}/fit_epos/{obs}_fit/pdf.txt", delimiter=","
            )
            ...
            neutrino_pdfs_mu_qgsjet = np.loadtxt(
                f"{geometry}/fit_qgsjet/{obs}_fit/pdf.txt", delimiter=","
            )
            ...
            neutrino_pdfs_mu_sibyll = np.loadtxt(
                f"{geometry}/fit_sibyll/{obs}_fit/pdf.txt", delimiter=","
            )
            # neutrino_pdfs_mub_sibyll = np.loadtxt(
            #     f"{geometry}/fit_sibyll/{obs}_fit/mub_pdf.txt", delimiter=","
            # )

            neutrino_pdfs_mu_dpm *= 2
            neutrino_pdfs_mu_epos *= 2
            neutrino_pdfs_mu_qgsjet *= 2
            neutrino_pdfs_mu_sibyll *= 2
            ...
            faser_pdf_mu_dpm, x_faser = read_pdf(pdf_dpm, x_vals, 12)
            faser_pdf_mub_dpm, x_faser = read_pdf(pdf_dpm, x_vals, -12)
            faser_pdf_mu_dpm = faser_pdf_mu_dpm * x_vals * factor
            faser_pdf_mub_dpm = faser_pdf_mub_dpm * x_vals * factor

            faser_pdf_mu_dpm += faser_pdf_mub_dpm

            pdf_epos = f"{formal_geometry}_EPOS+POWHEG_7TeV"
            faser_pdf_mu_epos, x_faser = read_pdf(pdf_epos, x_vals, 12)
            faser_pdf_mub_epos, x_faser = read_pdf(pdf_epos, x_vals, -12)
            faser_pdf_mu_epos = faser_pdf_mu_epos * x_vals * factor
            faser_pdf_mub_epos = faser_pdf_mub_epos * x_vals * factor
            faser_pdf_mu_epos += faser_pdf_mub_epos

            pdf_qgs = f"{formal_geometry}_QGSJET+POWHEG_7TeV"
            faser_pdf_mu_qgs, x_faser = read_pdf(pdf_qgs, x_vals, 12)
            faser_pdf_mub_qgs, x_faser = read_pdf(pdf_qgs, x_vals, -12)
            faser_pdf_mu_qgs = faser_pdf_mu_qgs * x_vals * factor
            faser_pdf_mub_qgs = faser_pdf_mub_qgs * x_vals * factor
            faser_pdf_mu_qgs += faser_pdf_mub_qgs

            pdf_sib = f"{formal_geometry}_SIBYLL+SIBYLL_7TeV"
            faser_pdf_mu_sib, x_faser = read_pdf(pdf_sib, x_vals, 12)
            faser_pdf_mub_sib, x_faser = read_pdf(pdf_sib, x_vals, -12)
            faser_pdf_mu_sib = faser_pdf_mu_sib * x_vals * factor
            faser_pdf_mub_sib = faser_pdf_mub_sib * x_vals * factor
            faser_pdf_mu_sib += faser_pdf_mub_sib
            ...
            # fig = plt.figure(figsize=(9.636, 8.0), dpi=300)  # 2 rows, 3 columns
            ...
            gs.update(
                left=0.08, right=0.99, top=0.92, bottom=0.08, hspace=0.15, wspace=0.15
            )
            ...
            plt.savefig(f"{geometry}_{obs}_compare_gens_elec.pdf")
            plt.show()
