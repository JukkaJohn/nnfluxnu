...
def plot(
    ...
    faser_pdf_mub, x_faser = read_pdf(pdf, x_vals, -pid, pdf_set)
    faser_pdf_mu = faser_pdf_mu * x_vals
    faser_pdf_mub = faser_pdf_mub * x_vals
    mean_fnu_mu = np.mean(neutrino_pdfs, axis=0) * x_vals
    ...
    # if preproc == 1:
    title_str = r"$\mathrm{Result} \ \mathrm{Closure} \ \mathrm{Test}$"
    # if preproc == 2:
    ...
    plt.savefig(f"{dir_for_data}/fit_result_comb_cl.pdf")
    plt.show()
