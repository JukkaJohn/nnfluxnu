...
def plot(
    ...
    for obs in observables:
        ...
        neutrino_pdfs_mu_bsm_eta = np.loadtxt(
            f"elec_bsm_eta/{obs}_fit/pdf.txt", delimiter=","
        )
        # neutrino_pdfs_mub_bsm_eta = np.loadtxt(
        #     f"elec_bsm_eta/{obs}_fit/mub_pdf.txt", delimiter=","
        # )

        neutrino_pdfs_mu_bsm_eta *= 2

        neutrino_pdfs_mu_bsm_eta_prime = np.loadtxt(
            f"elec_fnu_bsm_eta_prime/{obs}_fit/pdf.txt", delimiter=","
        )
        # neutrino_pdfs_mub_bsm_eta_prime = np.loadtxt(
        #     f"elec_fnu_bsm_eta_prime/{obs}_fit/mub_pdf.txt", delimiter=","
        # )
        neutrino_pdfs_mu_bsm_eta_prime *= 2
        neutrino_pdfs_mu_bsm_pi = np.loadtxt(
            f"elec_fnu_bsm_pi/{obs}_fit/pdf.txt", delimiter=","
        )
        neutrino_pdfs_mu_bsm_pi *= 2

        neutrino_pdfs_mu = np.loadtxt(
            f"new_run_3_gens/fit_epos/{obs}_fit/pdf.txt", delimiter=","
        )
        ...
        faser_pdf_mu_epos, x_faser = read_pdf(pdf, x_vals, 12, 2)
        faser_pdf_mub_epos, x_faser = read_pdf(pdf, x_vals, -12, 2)
        ...
        faser_pdf_mu_bsm_eta, x_faser = read_pdf(pdf_bsm_eta, x_vals, 12, 1)
        faser_pdf_mub_bsm_eta, x_faser = read_pdf(pdf_bsm_eta, x_vals, -12, 1)
        faser_pdf_mu_bsm_eta = faser_pdf_mu_bsm_eta * x_vals * factor
        faser_pdf_mub_bsm_eta = faser_pdf_mub_bsm_eta * x_vals * factor
        faser_pdf_mu_bsm_eta += faser_pdf_mub_bsm_eta

        pdf_bsm_eta_prime = "FASERv_Run3_BSM_eta_prime_7TeV"
        faser_pdf_mu_bsm_eta_prime, x_faser = read_pdf(pdf_bsm_eta_prime, x_vals, 12, 1)
        faser_pdf_mub_bsm_eta_prime, x_faser = read_pdf(
            pdf_bsm_eta_prime, x_vals, -12, 1
        )
        ...
        faser_pdf_mu_bsm_eta_prime += faser_pdf_mub_bsm_eta_prime

        pdf_bsm_pi = "FASERv_Run3_BSM_pi0_7TeV"
        faser_pdf_mu_bsm_pi, x_faser = read_pdf(pdf_bsm_pi, x_vals, 12, 1)
        faser_pdf_mub_bsm_pi, x_faser = read_pdf(pdf_bsm_pi, x_vals, -12, 1)
        faser_pdf_mu_bsm_pi = faser_pdf_mu_bsm_pi * x_vals * factor
        faser_pdf_mub_bsm_pi = faser_pdf_mub_bsm_pi * x_vals * factor
        faser_pdf_mu_bsm_pi += faser_pdf_mub_bsm_pi
        ...
        fig = plt.figure(figsize=(3.4, 6.4), dpi=300)  # 2 rows, 3 columns

        gs = gridspec.GridSpec(3, 1, height_ratios=[3, 2, 1])

        gs.update(
            left=0.19, right=0.94, top=0.92, bottom=0.08, hspace=0.15, wspace=0.15
        )
        ...
        # title_str = r"$\mathrm{EPOS} \ \mathrm{fluxes:} \ \mathrm{EPOS} + \ \mathrm{POWHEG}, \ \mathcal{L}_{\mathrm{pp}} = 150 \ \mathrm{fb}^{-1} \ \mathrm{FASER}\nu \ \mathrm{Run} \ 3$"
        ...
        axrL.set_ylabel(
            r"$\mathrm{Ratio} \ \mathrm{to} \ \mathrm{input} \ \mathrm{flux} $"
        )
        ...
        # axrL.set_xlabel(r"$x_\nu$")
        axrL.tick_params(
            which="both", left=True, right=True, bottom=True, top=True, direction="in"
        )
        axL.tick_params(
            which="both", left=True, right=True, bottom=True, top=True, direction="in"
        )
        ...
        axLsig.set_ylabel(r"$\mathrm{Pull} \ \mathrm{wrt} \ \mathrm{SM}$")
        ...
        # axMsig.plot(
        #     x_vals,
        #     np.abs(mean_mub_bsm_eta - faser_pdf_mub_epos) / error_mub_bsm_eta,
        #     linestyle="--",
        #     color="#fe6100",
        # )
        # # axMsig.fill_between(
        ...
        # axMsig.plot(
        #     x_vals,
        #     np.abs(mean_mub_bsm_pi - faser_pdf_mub_epos) / error_mub_bsm_pi,
        #     linestyle=":",
        #     color="#000000",
        # )
        # axMsig.fill_between(
        ...
        plt.savefig(f"{obs}_compare_bsm_elec.pdf")
        # plt.show()
