...
def get_data(
    ...
    print('shape fk table mu')
    print(fk_tables_mu.shape)
    if "flappertje" in filename_fk_mub_p:
        zero_row = torch.zeros((1, fk_tables_mu.shape[1]), dtype=fk_tables_mu.dtype, device=fk_tables_mu.device)

        fk_tables_mu = torch.cat([fk_tables_mu, zero_row], dim=0)
        
        fk_tables_mu = fk_tables_mu.view(-1, 2, fk_tables_mu.shape[1]).sum(dim=1)

        raw_data = data/binwidths
        new_binwidths = []
        for j in range(0,len(binwidths)-1,2):
            sum_binwidth = (binwidths_mu[j]/2 + binwidths_mu[j+1] /2) * (binwidths_mu[j+1]/binwidths_mu[j])
            new_binwidths.append(sum_binwidth)
        new_binwidths.append(binwidths_mu[-1])    
        binwidths_mu = new_binwidths
        binwidths_mu = np.array(binwidths_mu)
        print('shape bins')
        ...
        zero_row = torch.zeros((1, fk_tables_mub.shape[1]), dtype=fk_tables_mub.dtype, device=fk_tables_mub.device)

        fk_tables_mub = torch.cat([fk_tables_mub, zero_row], dim=0)

        fk_tables_mub = fk_tables_mub.view(-1, 2, fk_tables_mub.shape[1]).sum(dim=1)
...
def data_needed_for_fit(
    ...
    # faser_pdf, x = read_pdf(pdf, x_alphas.detach().numpy().flatten(), 12)
    # faser_pdf = torch.tensor(faser_pdf * 1.16186e-09, dtype=torch.float32)
    # data_mu = (
    #     (torch.matmul(fk_tables_mu, faser_pdf) * binwidths_mu)
    #     .detach()
    #     .numpy()
