...
neutrino_pdfs_mu_qgs_Eh = np.loadtxt("fit_qgsjet/Eh_fit/mu_pdf.txt", delimiter=",")
neutrino_pdfs_mub_qgs_Eh = np.loadtxt("fit_qgsjet/Eh_fit/mub_pdf.txt", delimiter=",")

neutrino_pdfs_mu_epos_Eh = np.loadtxt("fit_epos/Eh_fit/mu_pdf.txt", delimiter=",")
neutrino_pdfs_mub_epos_Eh = np.loadtxt("fit_epos/Eh_fit/mub_pdf.txt", delimiter=",")
...
def plot(
    ...
    mean_mu_qgs_Eh = np.mean(neutrino_pdfs_mu_qgs_Eh, axis=0) * x_vals
    error_mu_qgs_Eh = np.std(neutrino_pdfs_mu_qgs_Eh, axis=0) * x_vals
    mean_mub_qgs_Eh = np.mean(neutrino_pdfs_mub_qgs_Eh, axis=0) * x_vals
    error_mub_qgs_Eh = np.std(neutrino_pdfs_mub_qgs_Eh, axis=0) * x_vals

    mean_mu_epos_Eh = np.mean(neutrino_pdfs_mu_epos_Eh, axis=0) * x_vals
    error_mu_epos_Eh = np.std(neutrino_pdfs_mu_epos_Eh, axis=0) * x_vals
    mean_mub_epos_Eh = np.mean(neutrino_pdfs_mub_epos_Eh, axis=0) * x_vals
    error_mub_epos_Eh = np.std(neutrino_pdfs_mub_epos_Eh, axis=0) * x_vals
    ...
    axLqgs_Eh_err = axL.fill_between(
        x_vals,
        mean_mu_qgs_Eh + error_mu_qgs_Eh,
        mean_mu_qgs_Eh - error_mu_qgs_Eh,
        color="black",
        alpha=0.2,
        label=r"$\pm 1\sigma$",
    ...
    axLepos_Eh_err = axL.fill_between(
        x_vals,
        mean_mu_epos_Eh + error_mu_epos_Eh,
        mean_mu_epos_Eh - error_mu_epos_Eh,
        color="yellow",
        alpha=0.2,
        label=r"$\pm 1\sigma$",
    ...
    axL.legend(
        ...
        [
            ...
            (axLqgs_Eh_err, axLqgs_Eh),
            (axLepos_Eh_err, axLepos_Eh),
        ],
        [
            # r"$f_{\nu_\mu, \mathrm{ref \ (EPOS+PWG)}}$",
            # r"$f_{\nu_\mu, \mathrm{EPOS+POWHEG} \ E_\nu}$",
            r"$f_{\nu_\mu, \mathrm{DPM}\ E_h}$",
            r"$f_{\nu_\mu, \mathrm{QGS}\ E_h}$",
            r"$f_{\nu_\mu, \mathrm{EPOS}\ E_h}$",
        ],
    ...
    # if preproc == 1:
    title_str = r"$ Eh \ \mathrm{EPOS} \ + \ \mathrm{QGSJET} + \mathrm{DPMJET}$"
    # if preproc == 2:
    ...
    axMqgs_Eh_err = axM.fill_between(
        x_vals,
        (mean_mub_qgs_Eh + error_mub_qgs_Eh),
        (mean_mub_qgs_Eh - error_mub_qgs_Eh),
        color="black",
        alpha=0.2,
        label=r"$\pm 1\sigma$",
    ...
    axMepos_Eh_err = axM.fill_between(
        x_vals,
        (mean_mub_epos_Eh + error_mub_epos_Eh),
        (mean_mub_epos_Eh - error_mub_epos_Eh),
        color="yellow",
        alpha=0.2,
        label=r"$\pm 1\sigma$",
    ...
    axM.legend(
        ...
        [
            ...
            (axMqgs_Eh_err, axMqgs_Eh),
            (axMepos_Eh_err, axMepos_Eh),
        ],
        [
            # r"$f_{\bar{\nu}_\mu, \mathrm{ref} \ (EPOS+PWG)}$",
            # r"$f_{\bar{\nu}_\mu, \mathrm{EPOS+POWHEG} \ E_\nu}$",
            r"$f_{\bar{\nu}_\mu, \mathrm{DPM}\ E_h}$",
            r"$f_{\bar{\nu}_\mu, \mathrm{QGS}\ E_h}$",
            r"$f_{\bar{\nu}_\mu, \mathrm{EPOS}\ E_h}$",
        ],
        handler_map={tuple: HandlerTuple(ndivide=1)},
        loc="lower left",
    ).set_alpha(0.8)
    ...
    ratio_center = mean_mu_qgs_Eh / faser_pdf_mu
    ratio_lower = (mean_mu_qgs_Eh - error_mu_qgs_Eh) / faser_pdf_mu
    ratio_upper = (mean_mu_qgs_Eh + error_mu_qgs_Eh) / faser_pdf_mu
    ...
    ratio_center = mean_mu_epos_Eh / faser_pdf_mu
    ratio_lower = (mean_mu_epos_Eh - error_mu_epos_Eh) / faser_pdf_mu
    ratio_upper = (mean_mu_epos_Eh + error_mu_epos_Eh) / faser_pdf_mu
    ...
    axrM.plot(x_vals, mean_mub_qgs_Eh / faser_pdf_mub, linestyle="-", color="black")
    axrM.fill_between(
        x_vals,
        (mean_mub_qgs_Eh + error_mub_qgs_Eh) / faser_pdf_mub,
        (mean_mub_qgs_Eh - error_mub_qgs_Eh) / faser_pdf_mub,
        color="black",
        alpha=0.2,
    )

    axrM.plot(x_vals, mean_mub_epos_Eh / faser_pdf_mub, linestyle="-", color="yellow")
    axrM.fill_between(
        x_vals,
        (mean_mub_epos_Eh + error_mub_epos_Eh) / faser_pdf_mub,
        (mean_mub_epos_Eh - error_mub_epos_Eh) / faser_pdf_mub,
        color="yellow",
        alpha=0.2,
    )
