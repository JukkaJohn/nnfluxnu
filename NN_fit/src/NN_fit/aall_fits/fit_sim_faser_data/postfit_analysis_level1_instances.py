...
total_std_mu, total_std_mub, total_preds_Enu, total_std_preds_Enu = 0, 0, 0, 0
num_level1_instances = 10
...
for i in range(1, num_level1_instances + 1):
    ...
    rel_uncertainty = (
        np.std(neutrino_pdfs_mu, axis=0)
    ) / np.mean(neutrino_pdfs_mu, axis=0)
    ...
    total_preds_Enu += np.mean(N_event_pred, axis=0) / num_level1_instances
...
plt.title(r"$ \mathrm{pseudo \ \ data, \ \ 10 \ \ level \ \ 1 \ \ instances}$", loc="right")
...
plt.fill_between(x_vals.flatten(), (mean_rel_unc + std_rel_unc), (mean_rel_unc - std_rel_unc),color = 'red',label = 'rel unc')
#plt.legend()
plt.xlabel(r"$x_\nu$")
plt.xscale("log")
...
level0_fit = np.mean(N_event_pred, axis=0)
err_level0_fit = np.std(N_event_pred, axis=0)
