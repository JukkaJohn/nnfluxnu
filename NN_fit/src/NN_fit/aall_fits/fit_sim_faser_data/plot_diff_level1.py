...
def plot(
    ...
    axLvert1 = axL.axvline(x=100 / 7000, color="grey", linestyle="--", linewidth=1.5,  label="axvline - full height")
    axLvert2 = axL.axvline(x=1000 / 7000, color="grey", linestyle="--", linewidth=1.5,  label="axvline - full height")
    ...
    axMvert1 = axM.axvline(x=100 / 7000, color="grey", linestyle="--", linewidth=1.5,  label="axvline - full height")
    axMvert2 = axM.axvline(x=1000 / 7000, color="grey", linestyle="--", linewidth=1.5,  label="axvline - full height")
    axM.legend(
    ...
    axrL.axvline(x=100 / 7000, color="grey", linestyle="--", linewidth=1.5,  label="axvline - full height")
    axrL.axvline(x=1000 / 7000, color="grey", linestyle="--", linewidth=1.5,  label="axvline - full height")
    ...
    axrM.axvline(x=100 / 7000, color="grey", linestyle="--", linewidth=1.5,  label="axvline - full height")
    axrM.axvline(x=1000 / 7000, color="grey", linestyle="--", linewidth=1.5,  label="axvline - full height")
   ...
   # )
    ...
    axRlevel0err = axR.errorbar(
        Enumu_centers_plot,
        level0_fit / binwidths,
        yerr=err_level0_fit / binwidths,  # vertical error bars
        xerr=Enumu_errors_plot,  # horizontal error bars (bin widths)
        fmt="o",  # circle marker, set markersize to 0 to hide if needed
        markersize=3,
        color="black",
    ...
    axR.legend(
        [(axRpred), (axRsimerr), (axRlevel0err)],
        [
            # r"$\mathrm{DATA} \ E_\nu$",
    ...
    axR.set_title(r"$\mathrm{sim \ \ data}, \ \mathrm{Level\ 1}$", loc="left")
    ...
    axrRlevel0 = axrR.errorbar(
        Enumu_centers_plot,  # x values (bin centers)
        np.ones_like(simulated_Enu),  # y values (measurements)
        yerr=err_level0_fit / simulated_Enu,  # vertical error bars
        xerr=Enumu_errors_plot,  # horizontal error bars (bin widths)
        fmt="o",  # circle marker, set markersize to 0 to hide if needed
        markersize=3,
        color="black",
    ...
    axrR.legend(
        [(axrRpred), (axrRsimerr), (axrRlevel0,)],
        [
            # r"$\mathrm{DATA} \ E_\nu$",
            r"$\mathrm{FK} \otimes  f_{\nu_\mu, NN}\quad\mathrm{level 1s}$",
            r"$\mathrm{FK} \otimes f_{\nu_\mu, \mathrm{ref}}$",
