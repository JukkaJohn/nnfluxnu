...
for geometry_mu, geometry_el in zip(geometries_mu, geometries_el):
    for observable in observables:
        ...
        set_index += 1
        customize_info_file(template_path, path, set_index, "12,-14,14", num_members)
        with open(
            f"/Users/jukkajohn/Masterscriptie/faser_nufluxes_ml/ML_fit_enu/src/ML_fit_neutrinos/{geometry_el}/{observable}/pdf.txt"
        ) as f:
        ...
        with open(
            f"/Users/jukkajohn/Masterscriptie/faser_nufluxes_ml/ML_fit_enu/src/ML_fit_neutrinos/{geometry_mu}/{observable}/mu_pdf.txt"
        ) as f:
            lines = [line.strip().split(",") for line in f if line.strip()]
            numeric_data = [
                list(map(float, row)) for row in lines if is_numeric_row(row)
        ...
        with open(
            f"/Users/jukkajohn/Masterscriptie/faser_nufluxes_ml/ML_fit_enu/src/ML_fit_neutrinos/{geometry_mu}/{observable}/mub_pdf.txt"
        ) as f:
            lines = [line.strip().split(",") for line in f if line.strip()]
            numeric_data = [
                list(map(float, row)) for row in lines if is_numeric_row(row)
